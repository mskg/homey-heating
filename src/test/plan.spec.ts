...
import { DEFAULT_HEATING_PLAN, INTERMEDIATE, WARM, COOL } from "../app/helper/defaultPlan";
...
describe("Scheduler", () => {
    var scheduler = container.resolve(HeatingPlanCalculator);
    var plan: IHeatingPlan = _.find(DEFAULT_HEATING_PLAN, p => p.name == "Living Room");
    ...
    _.forEach(tests, test => {
        it(`Checking nextSchedule ${test.time.h}:${test.time.m}`, () => {
            ...
            var calcDate = scheduler.getNextSchedule(plan, ref);
        ...
        it(`Checking setPoint ${test.time.h}:${test.time.m}`, () => {
            ...
            var sp = scheduler.getSetPoint(plan, ref);
